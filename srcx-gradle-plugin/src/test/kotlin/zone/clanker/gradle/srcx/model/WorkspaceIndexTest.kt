            ...
            `when`("a scoped identity is requested") {
                then("it includes all declaration ownership") {
                    symbol.identity.value shouldBe
                        "root:::app::main::sample.Service#class sample.Service@src/main/kotlin/sample/Service.kt"
                }

                then("moving the declaration to another line preserves semantic identity") {
                    symbol.copy(declarationLine = 30).identity shouldBe symbol.identity
                }
            }
