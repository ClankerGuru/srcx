...
class SourceFileMetadataTest :
    BehaviorSpec({
        ...
        given("an ownership-filtered source file list") {
            val included = tempFile("Included", "kt", "package sample\nclass Included")
            val ignored = tempFile("Ignored", "txt", "not source")

            `when`("the exact files are parsed") {
                val results = scanSourceFiles(listOf(included, ignored, included))

                then("only distinct supported source files are returned") {
                    results.map { source -> source.simpleName } shouldBe listOf("Included")
