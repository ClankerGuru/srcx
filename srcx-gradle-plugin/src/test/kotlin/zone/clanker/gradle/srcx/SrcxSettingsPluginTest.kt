...
class SrcxSettingsPluginTest :
    BehaviorSpec({
        ...
        fun configureKotlinSourceRoots(project: Project) {
        ...
        fun captureRootTaskInputs(
            project: Project,
            task: ContextTask,
        ) {
            val layout = ProjectScanner.discoverSourceLayout(project)
            task.projectDirs.set(mapOf(project.path to project.projectDir))
            task.projectSourceLayouts.set(mapOf(project.path to layout))
            ...
            task.sourceFiles.setFrom(
                ProjectScanner.collectSourceFiles(project.projectDir, layout).map { source -> source.file } +
                    project.buildFile,
            )
        }
        ...
        given("extractProjectSummary") {
            ...
            `when`("project has source files") {
                ...
                configureKotlinSourceRoots(project)
        ...
        given("SettingsExtension") {
            ...
            fun newExtension(): Srcx.SettingsExtension =
                objects
                    ...
                    .also {
                        ...
                        it.projectWorkers.convention(Srcx.DEFAULT_PROJECT_WORKERS)
                    }
            ...
            `when`("project workers use the conservative default") {
                val extension = newExtension()

                then("four project jobs may overlap") {
                    extension.projectWorkers.get() shouldBe Srcx.DEFAULT_PROJECT_WORKERS
                }
            }

            `when`("project workers are configured") {
                val extension = newExtension()
                extension.projectWorkers.set(2)

                then("the typed value is retained") {
                    extension.projectWorkers.get() shouldBe 2
                }
            }
        }
        ...
        given("registerTasks") {
            ...
            `when`("registering tasks on a root project") {
                ...
                then("srcx-context has a safe project worker convention") {
                    val task = project.tasks.getByName(Srcx.TASK_CONTEXT) as ContextTask
                    task.projectWorkers.get() shouldBe Srcx.DEFAULT_PROJECT_WORKERS
                }

                then("executing srcx-context produces symbols and context") {
                    val task = project.tasks.getByName(Srcx.TASK_CONTEXT) as ContextTask
                    captureRootTaskInputs(project, task)
