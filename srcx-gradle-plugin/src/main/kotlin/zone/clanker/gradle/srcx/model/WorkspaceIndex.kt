...
data class WorkspaceSymbolIdentity(
    ...
    val value: String
        get() = "$build::$project::$sourceSet::$qualifiedName#$signature@$projectRelativeFile"
}

/** Report-safe declaration with full workspace ownership and exact source provenance when captured. */
...
data class WorkspaceSymbol(
    ...
    init {
        require(signature.isNotBlank()) { "workspace symbol signature must not be blank" }
        require(ownerQualifiedName == null || ownerQualifiedName.isNotBlank()) {
            "workspace symbol owner qualified name must not be blank"
        }
        require(ownerQualifiedName != qualifiedName) { "workspace symbol must not own itself" }
    }
