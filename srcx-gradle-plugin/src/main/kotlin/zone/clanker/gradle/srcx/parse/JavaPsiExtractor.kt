...
internal class JavaPsiExtractor {
    ...
    private fun extractClass(
        ...
        results.add(
            Symbol(
                ...
                signature = classSignature(kind, fqName),
                ownerQualifiedName = cls.containingClass?.qualifiedName,
                declarationRange = declarationRange(cls),
            ),
        )

        extractMethods(cls, fqName, pkg, file, results)
        extractFields(cls, fqName, pkg, file, results)
    ...
    private fun extractMethods(
        ...
        val className = cls.name ?: classQualifiedName.substringAfterLast('.')
        for (method in cls.methods) {
            val qualifiedName = "$classQualifiedName.${method.name}"
            results.add(
                Symbol(
                    ...
                    line = lineOf(method),
                    packageName = pkg,
                    signature = methodSignature(method, qualifiedName),
                    ownerQualifiedName = classQualifiedName,
                    declarationRange = declarationRange(method),
                ),
            )
        }
    }
    ...
    private fun extractFields(
        ...
        for (field in cls.fields) {
            val qualifiedName = "$classQualifiedName.${field.name}"
            results.add(
                Symbol(
                    ...
                    line = lineOf(field),
                    packageName = pkg,
                    signature = fieldSignature(field, qualifiedName),
                    ownerQualifiedName = classQualifiedName,
                    declarationRange = declarationRange(field),
                ),
            )
        }
    }
        ...
        imports.sortedBy { it.first.textOffset }.forEach { (statement, fqName) ->
            results.add(
                Reference(
                    ...
                    occurrenceRange = referenceRange(statement),
                ),
            )
        }
    }
        ...
        for (call in calls) {
            ...
            results.add(
                Reference(
                    ...
                    occurrenceRange = referenceRange(expression),
                ),
            )
        }
    }
        ...
        buildString {
            ...
            append(
                method.parameterList.parameters.joinToString(prefix = "(", postfix = ")") { parameter ->
                    parameter.typeElement?.text?.canonicalType() ?: "<unavailable>"
                },
            )
            method.returnTypeElement?.let { returnType ->
                append(':')
    ...
    private fun fieldSignature(
        ...
        val type = field.typeElement?.text?.canonicalType() ?: "<unavailable>"
        return "field $qualifiedName:$type"
    }
