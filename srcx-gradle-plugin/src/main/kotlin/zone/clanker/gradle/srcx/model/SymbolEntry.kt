 ...
 * @property signature normalized semantic signature used to distinguish overloads
 * @property ownerQualifiedName direct containing declaration, when PSI captures it
 * @property declarationRange exact UTF-16 declaration offsets, when PSI captures them
 */
...
data class Symbol(
    ...
    val signature: String = defaultSymbolSignature(kind, qualifiedName),
    val ownerQualifiedName: String? = null,
    val declarationRange: DeclarationRange? = null,
) {
    init {
        require(signature.isNotBlank()) { "symbol signature must not be blank" }
        require(ownerQualifiedName == null || ownerQualifiedName.isNotBlank()) {
            "symbol owner qualified name must not be blank"
        }
        require(ownerQualifiedName != qualifiedName) { "symbol must not own itself" }
    }
}

/** Exact UTF-16 offsets reported by PSI for a declaration. */
data class DeclarationRange(
    val startOffset: Int,
    val endOffsetExclusive: Int,
) {
    init {
        require(startOffset >= 0) { "declaration start offset must not be negative" }
        require(endOffsetExclusive > startOffset) { "declaration end offset must follow its start" }
    }
}

internal fun defaultSymbolSignature(
    kind: SymbolDetailKind,
    qualifiedName: String,
): String = "${kind.label} $qualifiedName"
 ...
 * @property occurrenceRange exact UTF-16 reference offsets, when PSI captures them
 */
data class Reference(
    ...
    val occurrenceRange: DeclarationRange? = null,
)
