...
class PsiParser(
    ...
    fun extractFacts(file: File): FileFacts = extractFacts(file, file.readText())

    /** Parse source text that was read before entering the shared PSI critical section. */
    fun extractFacts(
        file: File,
        sourceText: String,
    ): FileFacts =
        when (file.extension) {
            "kt", "kts" ->
                parseKtFile(file, sourceText)?.let { ktFile ->
                    FileFacts(
            ...
            "java" ->
                parseJavaFile(file, sourceText)?.let { javaFile ->
                    FileFacts(
