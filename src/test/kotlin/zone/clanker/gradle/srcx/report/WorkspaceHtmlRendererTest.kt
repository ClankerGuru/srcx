...
private fun assertAtlasVisualHierarchyContract(
    ...
    script shouldContain "var incident = (incidentLinksByNode.get(node.id) || []).slice()"
...
private fun assertAtlasFullscreenBehaviorContract(
    ...
    script shouldContain "setFullscreenChromeCollapsed(active && root.dataset.srcxFullscreenChrome === \"collapsed\")"
...
private fun assertAtlasNavigatorContract(
    ...
    assertAtlasNavigatorInteractionContract(script)
...
private fun assertAtlasNavigatorInteractionContract(script: String) {
    ...
    script shouldContain "var availableFileNodes = data.availableFileNodes || data.fileNodes"
    script shouldContain "var availableFileEdges = data.availableFileEdges || data.fileEdges"
    ...
    assertAtlasDirectManipulationContract(script)
}
...
private fun assertAtlasDirectManipulationContract(script: String) {
...
private fun assertAtlasLabelFitContract(
    ...
    script shouldContain "Fit shows the complete frame; zoom or hover to read labels on small nodes."
...
private fun assertAtlasScopedSymbolsAndPaletteContract(
    ...
    script shouldContain "availableSymbolNodes = data.availableNodes || data.nodes"
    script shouldContain "availableSymbolEdges = data.availableEdges || data.edges"
...
private fun assertAtlasSourceDetailResizeStyles(styles: String) {
    ...
    val detailResizeStyles =
        styles.substringAfter(".srcx-dashboard__architecture-detail-resize {").substringBefore("}")
    val sourceViewerStyles =
        styles.substringAfter(".srcx-dashboard__architecture-source-viewer {").substringBefore("}")
