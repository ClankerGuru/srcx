(function () {
    ...
    function enhanceGraph(root, graphIndex) {
        ...
        function selectedVisibleNodes() {
            return nodes.filter(function (node) { return state.selectedNodeIds.has(node.id); });
        }
        ...
        function graphFitRect(padding) {
            var detailOcclusion = 0;
            if (!detail.hidden && detail.classList.contains("is-open")) {
        ...
        function scopeDragStarted(event, scope, region) {
            markUserNavigation();
            var draggedNodes = nodes.filter(function (node) {
                return node.build === region.build && (scope === "build" || node.project === region.project);
            });
        ...
        function focusAdjacentNode(node, key) {
            var next = adjacentNode(node, key, nodes);
        ...
        function focusIncidentEdge(node) {
            var incident = (incidentLinksByNode.get(node.id) || []).slice().sort(function (left, right) {
                return left.id.localeCompare(right.id);
            });
